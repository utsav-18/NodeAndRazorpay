...
/* ctaX_full.js — full behavior for CTA (8-hour timer, animations, safe) */
(function () {
  'use strict';

  const DEFAULT_HOURS = 8;                  // <<-- 8 hours timer
  const PRICE_ID = 'ctaX_price_num';
  const HOURS_ID = 'ctaX_hours';
  const MINS_ID  = 'ctaX_mins';
  const SECS_ID  = 'ctaX_secs';
  const BTN_ID   = 'ctaX_action';

  // safe DOM getters
  const el = id => document.getElementById(id);

  const priceEl = el(PRICE_ID);
  const hoursEl = el(HOURS_ID);
  const minsEl  = el(MINS_ID);
  const secsEl  = el(SECS_ID);
  const btn     = el(BTN_ID);

  // sanity: if required bits missing, no-op
  if (!hoursEl || !minsEl || !secsEl) return;

  // ensure price displays ₹99
  if (priceEl) priceEl.textContent = '99';

  // prefers-reduced-motion check
  const prefersReduced = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  // compute epoch target from now
  function targetFromNow(hours) {
    const now = new Date();
    return new Date(now.getTime() + (hours * 3600 * 1000));
  }

  // animate small pop on digit change
  function popAnim(node) {
    if (!node) return;
    if (prefersReduced) return;
    node.classList.add('ani');
    // remove after duration (matches CSS .ani timing ~420ms)
    setTimeout(() => node.classList.remove('ani'), 420);
  }

  // timer state
  let target = targetFromNow(DEFAULT_HOURS);
  let intervalId = null;
  let expired = false;
  ...
  function updateClockOnce() {
    const now = new Date();
    let diff = Math.max(0, Math.floor((target - now) / 1000)); // seconds remaining
    ...
    // write and animate if changed
    if (hoursEl.textContent !== pad(h)) { hoursEl.textContent = pad(h); popAnim(hoursEl); }
    if (minsEl.textContent  !== pad(m)) { minsEl.textContent  = pad(m); popAnim(minsEl);  }
    if (secsEl.textContent  !== pad(s)) { secsEl.textContent  = pad(s); popAnim(secsEl);  }
  ...
  function startInterval() {
    ...
    // immediate paint
    updateClockOnce();
    intervalId = setInterval(updateClockOnce, 1000);
  }
  ...
  // reset timer to DEFAULT_HOURS from now
  function resetTimer() {
    expired = false;
    target = targetFromNow(DEFAULT_HOURS);
    if (btn) {
      btn.disabled = false;
      btn.style.opacity = '';
      // restore label if previously changed
      btn.textContent = btn.getAttribute('data-default-label') || btn.textContent || 'Enroll Now — ₹99';
    }
    // little visual pulse on reset
    if (!prefersReduced && hoursEl) popAnim(hoursEl);
    startInterval();
  }

  // initial setup: store default btn label
  if (btn && !btn.getAttribute('data-default-label')) {
    btn.setAttribute('data-default-label', btn.textContent.trim());
  }

  // button click: proxy primaryCTA if exists, else reset timer (and optionally navigate)
  if (btn) {
    btn.addEventListener('click', function (e) {
      // micro feedback
      if (!prefersReduced) {
        try {
          btn.animate([{ transform: 'scale(1)' }, { transform: 'scale(0.985)' }, { transform: 'scale(1)' }], { duration: 260, easing: 'ease-out' });
        } catch (err) { /* ignore */ }
      }
      ...
      // fallback: reset the timer and keep user on page (you can enable redirect here)
      resetTimer();

      // if you want auto-redirect to checkout, uncomment the next line and change the path:
      // window.location.href = '/payment/checkout';
    }, { passive: true });
  }

  // expose helper to global dev console
  window.ctaX = window.ctaX || {};
  window.ctaX.reset = resetTimer;
  window.ctaX.stop  = stopInterval;
  window.ctaX.start = startInterval;

  // start the timer loop
  startInterval();

  // keep update on visibilitychange to avoid big jumps when tab was inactive
  document.addEventListener('visibilitychange', function () {
    if (!document.hidden && !expired) updateClockOnce();
  });
